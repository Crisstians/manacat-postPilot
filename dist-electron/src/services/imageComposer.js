...
export const exportPostAssets = async (job) => {
    try {
        ...
        const fitted = resolveProductImageRect(productMeta.width ?? job.template.productLayer.width, productMeta.height ?? job.template.productLayer.height, job.template.productLayer, job.product.productImageLayout);
